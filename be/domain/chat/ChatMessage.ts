...
export class ChatMessage {
    ...
    static createFromAIResponse(roomId: string, response: AIChatAPIResponse): ChatMessage {
      const metadata = new ChatMetadata({
        totalFeedback: response.total_feedback.en,
        nativeTotalFeedback: response.total_feedback["ko-KR"],
      ...
      const evaluations = Object.entries(response.evaluation).map(([type, score]) => {
        const feedbackItems = response.feedback[type as keyof typeof response.feedback].map(item => 
