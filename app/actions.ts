...
export async function createAnonymousUser(): Promise<{
   ...
   return { sessionCreated: true, data };
}
export async function getOrCreateRoom({ user, session }: { user: User | null; session: Session | null }) {
   try {
      console.log(session);
      if (!user) {
